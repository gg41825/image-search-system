...
def seed_products(mongo: "MongoDBHandler", force_drop: bool = False) -> List:
    ...
    required_keys = {"id", "name", "category", "image_url"}
...
def seed_product_vectors_aligned(
    ...
    Supports caching and batch embedding.
    """

    os.makedirs(cache_dir, exist_ok=True)

    # Fetch product metadata for text and image embeddings
    ...
    # Generate cache key based on sample IDs
    key = ",".join(str(id) for id in sorted(sample_ids)) if sample_ids else "all"
    ...
    def is_cache_valid(path: str) -> bool:
        """Check if cache exists and not expired. The param for controlling the expiry days is cache_expiry_days"""
    ...
    if is_cache_valid(text_cache) and is_cache_valid(img_cache):
        ...
        # Encode images in batches (DINOv2)
    ...
    id_map: Dict[int, str] = {} # Annoy internal ID -> product ID
